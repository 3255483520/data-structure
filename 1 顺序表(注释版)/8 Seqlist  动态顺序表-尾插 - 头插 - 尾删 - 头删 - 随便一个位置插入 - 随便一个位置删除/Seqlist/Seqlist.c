...
{
	assert(pos < ps->size);//pos要小于size,只有这样才可以进行删除操作
	int start = pos + 1;//要把pos后面的数依次往前挪动1位，pos后面的数据就是pos+1,把start=pos+1位置的数据移动到pos位置处，后面依次操作
	while (start < ps->size)//start的值要小于size
	{
		ps->a[start - 1] = ps->a[start];//把pos后面的数据依次往前挪动1位
		++start;//start要加1
	}
	ps->size--;//便一个位置删除后，删掉了一个数据，所以总的有效数据减少了1个
