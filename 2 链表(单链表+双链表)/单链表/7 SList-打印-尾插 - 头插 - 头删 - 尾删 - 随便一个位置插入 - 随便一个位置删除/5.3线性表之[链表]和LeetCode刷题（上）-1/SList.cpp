...
{
	if (pos == *pphead)//这种情况相当于头插，直接调用头插的程序就行
	{
...
//删除pos位置的值
void SListErase(SLTNode** pphead, SLTNode* pos)//随便一个位置删除
{
	if (pos == *pphead)//这种情况相当于头删，直接调用头删的程序就行
	{
	...
	else//这种情况不是相当于头删的情况
	{
		...
		prev->next = pos->next;//把pos位置free掉之前，因为需要把pos后面的一个节点的地址和pos前面的prev指向的next连接在一起，所以必须把pos指向的next
		//保存下来，pos指向的next就是是pos后面一个节点的地址
		//prev是pos前面的一个，prev->next就是prev指向的next
		//综上所述：prev->next = pos->next
		free(pos);
	}
}